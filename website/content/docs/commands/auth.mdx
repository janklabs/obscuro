...
<Callout type="info">
  When you run `obscuro auth store`, an interactive selector lets you pick your
  password backend (OS keychain or managed file). Run `obscuro auth doctor` to
  see which backends are available first.
</Callout>
...
<Callout type="info">
  Prefer `obscuro auth doctor` first — it prints exactly which backends are
  working right now with only transient probe side effects.
</Callout>
